...
#rects <- geom_rect(data=newdata, aes(xmin=xmin, xmax=xmax, ymin=-8, ymax=3, fill="none"), color="black", alpha=0.5)
eralines <- geom_vline(xintercept = newdata$xmax, alpha=0.25)
eralabels <- geom_text(data=newdata, x=newdata$xmax, y=4, label=newdata$fed.High.Value, angle=90, size=4)
...
#p4 <- geom_line(aes(x=Year, y=fed_diff, col=State))
